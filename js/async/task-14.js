...
// Создайте асинхронную функцию myThirdAsyncFunction которая должна вызвать функцию sendRequest для каждого урла и вернуть объект 
// {name: 'Ann', age: 16}.
// Обработайте результат работы функции myThirdAsyncFunction.

const arrayUrl = ['http://a', 'http://b'];
...
async function myThirdAsyncFunction() {
    const obj1 = await sendRequest(arrayUrl[0]);
    const obj2 = await sendRequest(arrayUrl[1]);
